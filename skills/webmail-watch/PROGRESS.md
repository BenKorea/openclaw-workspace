...
- [x] **P8.3 데스크탑(kimbi) 첫 운영 + timing race fix** *(2026-05-15 완료)*
  ...
  - **fix (2026-05-15)**: `run.py` 의 `open_context` 에 `slow_mo=int(os.environ.get("WEBMAIL_SLOW_MO", "300"))` 추가. default 300ms — race 안전망 + poll_limit=3 회차당 5~10초 추가 (무해). env 로 0 override 가능 (디버깅·노트북 OFF). *(아래 2026-05-17 후속에서 300 불충분 판명·800 상향.)*
  - **후속 (2026-05-17) — 300ms 불충분, demo-검증값 800 으로 상향**: Telegram 수동 `/webmail_watch kirams --limit N` 이 `auth_failed`. OpenClaw 에이전트는 *"KIRAMS resource-servers 502 외부장애, 복구대기 유일"* 로 단정 진단했으나 — **demo(headed+slow_mo=800)가 동시기 정상 forwarding** 이라는 Dr. Ben 반증으로 오진 판명. 실제는 동일 **timing race 재발**: cron 경로(headless·slow_mo 기본 300)가 KIRAMS reactive-UI/resource-servers 부트스트랩보다 빨라 race → SPA 빈 화면 → `is_logged_in`/`perform_login` 가 inbox_marker 부재로 `auth_failed` 오분류(502 는 race 의 *증상*). 코드 델타 = `headless`(cron필수·보정불가)·`slow_mo`(300 vs 800)뿐, `perform_login`/`process_inbox` 는 demo 가 run.py import = 동일. **fix**: `open_context` slow_mo 기본 `300`→`800`(demo-검증값). 백그라운드 폴러라 +Δ 운영 무의미. demo-전용 과함은 headed/trace/walkthrough(cron 에 애초 없음)이지 slow_mo 가 아님 → slow_mo 800 은 정당한 안전값. 교훈: 에이전트 단일-경로 자신만만 진단의 위험(502→"서버 다운, 할 일 없음" 잘못된 수동대기 권고) — demo vs 실경로 *통제 비교*가 진단 정답. 검증법: KIRAMS 도달 가능 시 `WEBMAIL_SLOW_MO=800 uv run run.py kirams --limit 1`(headless 유지) 성공 = 타이밍 확정·headless-고유 배제. 추후 cron 시간 단축 필요 시 *측정 기반*(binary search)으로만 하향(추측 하향 = 재발). **✅ 검증 완료 (2026-05-17 16:26, ai4lt)**: 위 명령 headless 그대로 → `📨 신규 1건 forwarding 완료`(~20-30초). 타이밍 확정·headless-고유 배제·에이전트 502-외부장애 진단 오진 확정. slow_mo 800 수정 실측 입증.
