...
| `6 할일` | Task | 〃 | → `9 완료` |
| `4 일정+회신` | Calendar + 초안 | 〃 | 유지 → 발송 감지 시 `9 완료` |
| `5 일정+할일+회신` | Calendar + Task + 초안 | 〃 | 유지 → 발송 감지 시 `9 완료` |
| `7 할일+회신` | Task + 초안 | 〃 | 유지 → 발송 감지 시 `9 완료` |
| `8 회신` | 초안 | 〃 | 유지 → 발송 감지 시 `9 완료` |

**sent-poll 레인 (Phase 2, 2026-07-13 구현)**: 라벨 드레인 후 `in:sent newer_than:2d`(env `GMAIL_SENT_WINDOW`) 폴링 —
① 캡처 폴더가 있는 스레드에 내 발신이 늘었으면 **재캡처**(멱등, `_actions.json` 보존) → 내 회신이 `_thread.md` 에 반영.
② 회신형 라벨 스레드의 마지막 메시지가 나(SELF)면 **자동 종결**(`9 완료`).
③ 미캡처·무라벨 발신 스레드는 **보고만** — 목표는 *캡처된 사안의 스레드 완전성*이지 모든 발신 보관이 아님(선별 원칙). 편입하려면 발신 메일에 라벨을 붙이거나(검색이 sent 도 잡음) brainify 지시. `GMAIL_SENT_POLL=off` 로 레인 비활성.
...
- **잡첨부 필터** (2026-06-07): 첨부 저장 시 노이즈 제거. **inline 이미지**(image/* + Content-Disposition:inline 또는 Content-ID; 또는 이름 `image\d+\.(png|gif|jpg)`) = **완전 삭제**. **TNEF 봉투**(`application/ms-tnef`·`winmail.dat`·매직 `78 9f 3e 22`) = **내부 실첨부가 추출됐을 때만 삭제**(봉투가 유일 원본이면 보존). 판별은 thread payload parts 메타(`--full` 로 이미 받음) + 파일 매직. 결정형·무판단. 서명 로고·중복 봉투가 sources 에 안 들어옴.
...
- **회신 생애주기 (Phase 2 완결)**: 초안 생성(라벨 유지) → Dr. Ben 검토·발송 → sent-poll 이 발송 감지 → 재캡처 + 자동 `9 완료`. 한계: **Gmail 발신만** 감지 — KIRAMS 웹메일 발신은 범위 밖(그 사안은 self-forward 관행으로 보완).
...
- sent-poll 재캡처도 저장 디렉토리 범위 — 이미 PARA 로 이동한 사안의 후속 발신은 "미캡처 보고" 로만 뜸 → 소비 시점 신선도 체크(brainify·close 스킬)가 최종 방어.
- sent-poll 은 Gmail 계정 발신만 봄 — KIRAMS 웹메일 발신 미포함.
